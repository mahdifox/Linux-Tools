...
if [[ ! -z $arg1 ]]
then
	# Deleting DNS1 & DNS2 lines in ifcfg-eth0 file.
	if [ -f "/etc/sysconfig/network-scripts/ifcfg-eth0" ]
	then	
		if grep -q "^DNS" "/etc/sysconfig/network-scripts/ifcfg-eth0"
		then
			sed -i '/^DNS/d' "$MY_PATH/ifcfg-eth0";
		fi	
	fi
	...
	if [ -f "/usr/bin/chattr2" ]
	then
		chattr2 -i "/etc/resolv.conf";
	fi
