...
then
        dbuser="cpanel";
        dbpass="cpanel";
        mysql -e 'show databases;' | grep -v "|" | grep -v "Database\|information_schema\|performance_schema\|mysql" > $MY_PATH/all_dbs;
...
then
        ...
        echo -e "${RED} failed: No cpanel or directadmin Panel detected ${NC}";
        exit 1  # fail
fi
...
then
        ...
        then
                ...
                do
                        ...
                        then
                                mysqlcheck --auto-repair $line && mysqlcheck --optimize $line && mysqlcheck --check $line;
                                if [ $? -eq 0 ]
                                then
